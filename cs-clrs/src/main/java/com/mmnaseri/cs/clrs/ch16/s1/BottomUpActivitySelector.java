...
public class BottomUpActivitySelector implements ActivitySelector {
    ...
    public Set<Integer> select(Activity... activities) {
        //we need to remember the original position of the activities, so that when we are reporting them back
        ...
        //this matrix will hold the count of the longest selection of activities starting with a[i] and ending with a[j]
        //in selection[i][j]. Since we only need the upper half for this, we use the lower half in symmetric form to
        //store the location of one of the activities between a[i] and a[j], a[k], such that a[k] is compatible with
        ...
        //this is for pointing the latest found longest activity planning sequence
    ...
    @Complexity(value = "O(n^3)", explanation = "We are ranging three cursors over n in three nested loops")
        ...
        for (int length = 1; length <= indexedActivities.length; length ++) {
            ...
            for (int i = 0; i < indexedActivities.length - length; i++) {
                ...
                for (int k = i + 1; k < j; k ++) {
                    ...
                    if (indexedActivities[i].getFinish() > indexedActivities[k].getStart() || indexedActivities[k].getFinish() > indexedActivities[j].getStart()) {
                        continue;
                    }
    ...
    @Complexity(value = "O(n)", explanation = "Because we are at most visiting each range once, and no two ranges overlap except at the beginning and the end")
    private Set<Integer> collectIndexes(int[][] selection, int start, int end) {
        ...
        if (end - start > 1) {
            ...
            if (middle != -1) {
                //if midpoint is not `-1` then activity a[middle] exists such that a[middle] is compatible with
