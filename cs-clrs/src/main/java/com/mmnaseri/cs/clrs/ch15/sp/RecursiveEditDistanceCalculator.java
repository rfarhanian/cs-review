...
public class RecursiveEditDistanceCalculator implements EditDistanceCalculator {
    ...
    private List<EditOperation> calculate(String source, String target, int sourceIndex, int targetIndex, CostFunction function) {
        ...
        if (sourceIndex >= 1 && targetIndex >= 1 && source.charAt(sourceIndex) == target.charAt(targetIndex - 1) && source.charAt(sourceIndex - 1) == target.charAt(targetIndex)) {
            final List<EditOperation> prefix = calculate(source, target, sourceIndex - 2, targetIndex - 2, function);
        ...
        if (source.charAt(sourceIndex) == target.charAt(targetIndex)) {
            final List<EditOperation> prefix = calculate(source, target, sourceIndex - 1, targetIndex - 1, function);
            int localCost = EditOperation.getTotalCost(prefix);
        ...
        for (EditOperationType type : Arrays.asList(EditOperationType.DELETE, EditOperationType.INSERT, EditOperationType.REPLACE)) {
            ...
            final List<EditOperation> prefix = calculate(source, target, sourceIndex - type.getSource(), targetIndex - type.getTarget(), function);
            int localCost = EditOperation.getTotalCost(prefix);
